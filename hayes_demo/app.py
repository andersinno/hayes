...
class SearchForm(forms.Form):
    ...
    use_match_query = forms.BooleanField(
        label="Match Query",
        help_text=u"Better results, but no query strings",
        required=False, initial=True)
    only_people = forms.BooleanField(
        label=u"Only search people", required=False)
    person_height_min = forms.IntegerField(
        label=u"Person height minimum", required=False)
    person_height_max = forms.IntegerField(
        label=u"Person height maximum", required=False)
...
class SearchView(TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if complete:
            ...
            result = get_connection().completion_suggest(
                page_index, complete, fuzzy=2)
            return HttpResponse(content=json.dumps(
                result.options), content_type="application/json")
